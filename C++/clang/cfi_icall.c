...
typedef int (*int_arg_fn)(int);          //方法指针
...
struct foo {
    int_arg_fn int_funcs[1]; //[1]代表是数组          按照顺序
...
//传入1的时候，int_funcs[1](1)，参数是1的方法指针，指向foo里的[1]第二个方法，bad_int_arg，调用返回 printf("CFI will not protect transfer to here\n");
//传入2的时候，int_funcs[2](2)，指向foo里的[2]个方法float_arg，此时入参为int 声明为float_arg(float arg)，不一致，故返回runtime error: control flow integrity check for type 'int (int)' failed during indirect function call